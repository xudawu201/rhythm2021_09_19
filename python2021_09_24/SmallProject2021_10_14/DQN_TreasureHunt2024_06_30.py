...
def main():
    ...
    # 保存模型路径和名字
    # actor模型路径
    QNetTrainModelFilePath_str = 'QNetTrain.model'
    QNetTargetModelFilePath_str = 'QNetTarget.model'
    ...
    # 初始化用于决策的Q网络
    QNetTrain_model = Actor(input_size, out_features)
    # 初始化目标Q网络，用于稳定学习过程
    QNetTarget_model = Actor(input_size, out_features)
    # 将训练Q网络的参数复制给目标Q网络
    QNetTarget_model.load_state_dict(QNetTrain_model.state_dict())

    # 加载模型
    # QNetTrain_model=torch.load(QNetTrainModelFilePath_str)
    # QNetTarget_model=torch.load(QNetTargetModelFilePath_str)
    ...
    for epoch_int in range(totalEpoch_int):
        ...
        # 每最大步数次之后或者游戏结束后训练更新网络
        while not done:
            #返回概率tensor
            x_tensor = DQNAgent_model.QNetTrain_model(sInputDevice_tensor)
            # 创建概率分布模型,然后利用这个分布进行采样、计算对数概率、熵等操作,不需要计算梯度使用detach()截断梯度
            ...
            # 根据当前状态和epsilon选择动作
            action_int = DQNAgent_model.sampleAction(x_tensor,out_features,epsilon)
            ...
            DQNAgent_model.saveMemory(state_tensor, action_int, reward_float, nextState_tensor,done)
            ...
            #打印当前步信息
            # print('step:',step_int,'action:',action_dict.get(action_int),'reward:',reward_float,'epsilon:',epsilon,'prob:',prob[action_int].item(),done)
        ...
        # 存储模型
        # 模型转到cpu
        # DQNAgent_model.QNetTrain_model=DQNAgent_model.QNetTrain_model.to('cpu')
        # DQNAgent_model.QNetTarget_model=DQNAgent_model.QNetTarget_model.to('cpu')
        # # 存储actor模型
        # torch.save(DQNAgent_model.QNetTrain_model,QNetTrainModelFilePath_str)
        # torch.save(DQNAgent_model.QNetTarget_model,QNetTargetModelFilePath_str)
        # # 存储完成后再转到gpu继续训练
        # DQNAgent_model.QNetTrain_model=DQNAgent_model.QNetTrain_model.to(device)
