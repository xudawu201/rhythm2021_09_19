...
LastEditors: xudawu
LastEditTime: 2024-06-25 23:26:59
'''
...
def main():
    ...
    for epoch_int in range(totalEpoch_int):
        ...
        sInput_tensor=img_tensor.view(
            ...
            img_tensor.shape[1],
            img_tensor.shape[2])
        sInput_tensor=sInput_tensor.to(device)
        window.update_idletasks()  # 更新界面
        window.update()  # 更新并等待一小段时间让界面反应
        ...
        for step_int in range(maxStep_int):
            #返回概率tensor
            x_tensor = ppoAgent_model.actor(sInput_tensor)
            prob = torch.nn.functional.softmax(x_tensor, dim=1)
            ...
            # 下一个环境数据转到device
            sNextInput_tensor=sNextInput_tensor.to(device)
            # nextState等于玩家和宝藏坐标
            ...
            prob_float = prob[0][a].item()
            stateSave_tensor=sInput_tensor[0]
            sNextSave_tensor=sNextInput_tensor[0]
            print('a:',a, 'reward:',r,'prob_float:',prob_float)
            ppoAgent_model.saveMemory(stateSave_tensor, a, totalReward_float, sNextSave_tensor, prob_float, done)
