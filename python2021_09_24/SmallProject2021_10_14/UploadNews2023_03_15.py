...
LastEditors: xudawu
LastEditTime: 2023-03-18 22:19:24
'''
...
# 点击国家热点信息
def clickHotInf(country_str,columnAddDiv_WebDriver,hotInfoCount_int):
    ...
    try:
        ...
        if thisCountry_name in countries:
            ...
            if index==0:
                # 等待加载完成
                findInfo_str='/html/body/div['+str(hotInfoCount_int)+']/div/ul/li[2]/ul/li[5]/ul/li[7]'
                pageLocator_str=('xpath',findInfo_str)
                waitPageLoaded(pageLocator_str)
                # 如果不在列表中,直接热点信息
                # 切换窗口
                hotInfo_WebDriver=browser_WebDriver.find_element('xpath',findInfo_str)
                hotInfo_WebDriver=hotInfo_WebDriver.find_element('tag name','div')
                browser_WebDriver.execute_script("arguments[0].scrollIntoView();", hotInfo_WebDriver)
                # 使用js实现点击
                browser_WebDriver.execute_script('arguments[0].click();', hotInfo_WebDriver)
                hotInfoCount_int=hotInfoCount_int+1
                return hotInfoCount_int
            # 泰国
            if index==1:
                # 等待加载完成
                findInfo_str='/html/body/div['+str(hotInfoCount_int)+']/div/ul/li[2]/ul/li[6]/ul/li[7]'
                pageLocator_str=('xpath',findInfo_str)
                waitPageLoaded(pageLocator_str)
                # 如果不在列表中,直接热点信息
                # 切换窗口
            ...
            if index==2:
                # 等待加载完成
                findInfo_str='/html/body/div['+str(hotInfoCount_int)+']/div/ul/li[2]/ul/li[7]/ul/li[7]'
                pageLocator_str=('xpath',findInfo_str)
                waitPageLoaded(pageLocator_str)
                # 如果不在列表中,直接热点信息
                # 切换窗口
            ...
            if index==3:
                # 等待加载完成
                findInfo_str='/html/body/div['+str(hotInfoCount_int)+']/div/ul/li[2]/ul/li[8]/ul/li[7]'
                pageLocator_str=('xpath',findInfo_str)
                waitPageLoaded(pageLocator_str)
                # 如果不在列表中,直接热点信息
                # 切换窗口
            ...
            if index==4:
                # 等待加载完成
                findInfo_str='/html/body/div['+str(hotInfoCount_int)+']/div/ul/li[2]/ul/li[9]/ul/li[7]'
                pageLocator_str=('xpath',findInfo_str)
                waitPageLoaded(pageLocator_str)
                # 如果不在列表中,直接热点信息
                # 切换窗口
            ...
            if index==5:
                # 等待加载完成
                findInfo_str='/html/body/div['+str(hotInfoCount_int)+']/div/ul/li[2]/ul/li[10]/ul/li[7]'
                pageLocator_str=('xpath',findInfo_str)
                waitPageLoaded(pageLocator_str)
                # 如果不在列表中,直接热点信息
                # 切换窗口
            ...
            if index==6:
                # 等待加载完成
                findInfo_str='/html/body/div['+str(hotInfoCount_int)+']/div/ul/li[2]/ul/li[11]/ul/li[7]'
                pageLocator_str=('xpath',findInfo_str)
                waitPageLoaded(pageLocator_str)
                # 如果不在列表中,直接热点信息
                # 切换窗口
            ...
            if index==7:
                # 等待加载完成
                findInfo_str='/html/body/div['+str(hotInfoCount_int)+']/div/ul/li[2]/ul/li[12]/ul/li[7]'
                pageLocator_str=('xpath',findInfo_str)
                waitPageLoaded(pageLocator_str)
                # 如果不在列表中,直接热点信息
                # 切换窗口
            ...
            if index==8:
                # 等待加载完成
                findInfo_str='/html/body/div['+str(hotInfoCount_int)+']/div/ul/li[2]/ul/li[13]/ul/li[7]'
                pageLocator_str=('xpath',findInfo_str)
                waitPageLoaded(pageLocator_str)
                # 如果不在列表中,直接热点信息
                # 切换窗口
            ...
            if index==9:
                # 等待加载完成
                findInfo_str='/html/body/div['+str(hotInfoCount_int)+']/div/ul/li[2]/ul/li[14]/ul/li[7]'
                pageLocator_str=('xpath',findInfo_str)
                waitPageLoaded(pageLocator_str)
                # 如果不在列表中,直接热点信息
                # 切换窗口
            ...
            # time.sleep(1)
            # browser_WebDriver.switch_to.default_content()
            # browser_WebDriver.switch_to.frame('main')
            # 等待加载完成
            findInfo_str='/html/body/div['+str(hotInfoCount_int)+']/div/ul/li[2]/ul/li[4]'
            pageLocator_str=('xpath',findInfo_str)
            waitPageLoaded(pageLocator_str)
            # 如果不在列表中,直接热点信息
            # 切换窗口
...
# 控制页面提交新闻信息
def controlPage(country_str,cuntryAgency_str,newIndex_int,newTitle_str,newUrl_str,contentList_list,hotInfoCount_int):
    ...
    newContent_WebDriver.send_keys(Keys.ENTER)
    # 定位到文本首行
    newContent_WebDriver.send_keys(Keys.CONTROL,Keys.PAGE_UP)
    ...
    # 点击热点信息
    hotInfoCount_int=clickHotInf(country_str,columnAddDiv_WebDriver,hotInfoCount_int)
...
if __name__ == '__main__':
    ...
    for contentList_index in range(len(news)):
        ...
        hotInfoCount_int=controlPage(country_str,countryAgency_str,newIndex_int,newTitle_str,newUrl_str,contentList_list,hotInfoCount_int)
        # 每上传一次新闻暂停
        time.sleep(3)
